...
def _remove_outliers(objects, m=2.0):
    ...
    coords = np.array([[obj.x, obj.y] for obj in objects])
...
def _extract_frame_features(frame_data, velocity_thresh=0.1):
    ...
    """
    body = frame_data.objects if hasattr(frame_data, 'objects') else []
    
    num_objs = frame_data.num_objects if hasattr(frame_data, 'num_objects') else 0
    if num_objs == 0:
        return [0, 0, 0, 0, 0, 0, 0, 0, 0]
    
    # Initialize defaults
    avg_x = range_x = avg_y= std_vel = max_snr = avg_snr = avg_rcs = max_rcs = spatial_extent = 0.0
    ...
    if body:
        ...
        moving_objs = [
            obj for obj in body if 
            abs(obj.velocity) > velocity_thresh 
            and abs(obj.x) < x_ceil
            #and obj.snr > snr_threshold
			#and obj.snr < snr_ceil
        ]

        moving_objs = _remove_outliers(moving_objs, m=1.0)
        
        if len(moving_objs) > 2: # DBSCAN needs at least 3 points for min_samples=3
                coords = np.array([[obj.x, obj.y] for obj in moving_objs])
        ...
        if moving_objs:
            xvals = np.array([obj.x for obj in moving_objs])
            yvals = np.array([obj.y for obj in moving_objs])
            velocities = np.array([obj.velocity for obj in moving_objs])
            snr_vals = np.array([obj.snr for obj in moving_objs])
...
def _aggressive_smoothing(data, window_length=15, polyorder=3):
    """
    Applies aggressive smoothing using a Savitzky-Golay filter.
    This is preferred for position data before differentiation (acceleration)
    because it preserves signal peaks better than a simple moving average.
    ...
    if len(data) < window_length:
        return data # Not enough data to smooth effectively
    
    # Ensure window_length is odd as required by savgol_filter
...
def dataprep_queue(data_queue, smooth_window=5):
    """
    ...
    Returns:
        list: A 2D list where each row is a feature vector for a single frame.
              Features: [num_objs, avg_x, range_x, avg_vel, std_vel, max_vel, min_vel, max_snr, accel]
              Returns None if file not found or error occurs.
    """
    
    if len(data_queue) == 0:
        print ("Warning: Data queue is empty.")
        return None
    ...
    all_frame_features = []
    for frame_data in list(data_queue):
...
def data_diagnostic(
    queue,
    num_features,
    feature_names=['num_objs', 'avg_x', 'range_x', 'avg_y', 'avg_vel', 'std_vel', 'max_vel','min_vel','max_snr','avg_snr','vert_vel','accel']
):
    
    print("\n--- Features Diagnostic ---")
    features = {i: [] for i in range(num_features)}

    raw = dataprep_queue(queue)
    if raw is None:
